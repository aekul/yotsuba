...
struct Raycast {
	Raycast(const Scene *scene) : scene(scene) {}

	/// For environment map
	inline BSphere getBSphere() const {
		BSphere bsphere = scene->getBSphere();
		bsphere.radius = std::max(Epsilon, bsphere.radius * 1.5f);
		return bsphere;
	}

	inline Intersection operator()(const Ray& ray, bool isValid = true) const {
		Intersection its;
	    if (!isValid) {
	      return its;
	    }
	    ...
	    // its.isTriangle cannot be true if we hit nothing
	    SAssert(hit || (!hit && !its.isTriangle));
	    if (hit) {
	    	SAssert(its.shape != nullptr);
	    	const BSDF *bsdf = its.getBSDF();
	    	SAssert(bsdf != nullptr);
	    	// for bump map
	    	its.shFrame = bsdf->getFrame(its);
	    	// if we want to implement displacement map, do something like
	    	// its = bsdf->getIntersection(its);
	    	its.emitter = its.shape->getEmitter();
	    } else {
	    	BSphere bsphere = getBSphere();
	    	its.sph_center = bsphere.center;
	    	its.sph_radius = bsphere.radius;
	    	its.time = ray.time;
	    	its.emitter = scene->getEnvironmentEmitter();
	    }
	    return its;
	}
