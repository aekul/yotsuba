...
public:
	...
	// Used by aether, saying that the intersection stored here is already perturbed and doesn't need to be perturbed again
	bool framePerturbed;
};
...
public:
	...
	/// Return (approximated) f/p assuming perfect importance sampling
	...
	virtual bsdf_component_sampler makeSampler(const Vector &wiLocal, const Intersection &its, const uint32_t componentMask) const {
		bsdf_component_sampler sampler;
		sampler.AddComponent(cosine_hemisphere_sampling{}, Real(1));
		return sampler;
	}
