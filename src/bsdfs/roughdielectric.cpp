...
public:
	...
	Float pdf(const BSDFSamplingRecord &bRec, EMeasure measure) const {
		...
		if (hasTransmission && hasReflection) {
			Float F = fresnelDielectricExt(Frame::cosTheta(bRec.wi), m_eta);
			prob *= reflect ? F : (1-F);
		}
	...
	Spectrum sample(BSDFSamplingRecord &bRec, const Point2 &_sample) const {
		...
		weight = eval(bRec, ESolidAngle) / this->pdf(bRec, ESolidAngle);
	...
	bsdf_component_sampler makeSampler(const Vector &wiLocal, const Intersection &its,
			const uint32_t componentMask) const {
		SAssert(m_type == MicrofacetDistribution::EBeckmann);
		// TODO: support anisotropic bsdf
		Float alpha = m_alphaU->eval(its).average();
		// Note that this is an approximation: Mitsuba uses micronormal for Fresnel evaluation
		// while we use macronormal.  It's an approximation since it's easier to implement
		// (don't have to implement symbolic Fresnel...)
		// It shouldn't be a bad approximation when alpha is small.
		// Beckmann BSDF sucks at large alpha anyway (unless we want to implement Heitz's multiple-scattering microfacet).
		
		bool doTransmission = componentMask & EGlossyTransmission;
		bool doReflection = componentMask & EGlossyReflection;
		if (doTransmission && !doReflection) {
			bsdf_component_sampler sampler;
			sampler.AddComponent(beckmann_sampling{alpha, m_eta, beckmann_sampling::REFRACT}, Real(1));
			return sampler;
		}
		if (doReflection && !doTransmission) {
			bsdf_component_sampler sampler;
			sampler.AddComponent(beckmann_sampling{alpha, m_eta, beckmann_sampling::REFLECT}, Real(1));
			return sampler;
		}

		Float F = fresnelDielectricExt(wiLocal[2], m_eta);
		bsdf_component_sampler sampler;
		if (F < 1) {
			sampler.AddComponent(beckmann_sampling{alpha, m_eta, beckmann_sampling::REFRACT}, Real(1 - F));
		}
		if (F > 0) {
			sampler.AddComponent(beckmann_sampling{alpha, m_eta, beckmann_sampling::REFLECT}, Real(F));
		}
		return sampler;
	}
