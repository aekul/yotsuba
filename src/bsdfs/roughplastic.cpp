...
public:
	...
	bsdf_component_sampler makeSampler(const Vector &wiLocal, const Intersection &its,
		...
		Float alpha = m_alpha->eval(its).average();
		Float probSpecular = 1 - m_externalRoughTransmittance->eval(Frame::cosTheta(wiLocal), alpha);
		...
		if (probSpecular < 1) {
			sampler.AddComponent(cosine_hemisphere_sampling{}, Real(1 - probSpecular));
		}
		if (probSpecular > 0) {
			sampler.AddComponent(beckmann_sampling{alpha, 1.0, beckmann_sampling::REFLECT}, Real(probSpecular));
		}
		return sampler;
	}
