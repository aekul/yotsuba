...
public:
	...
	static void set(Properties &props, const std::string name, bp::object value) {
		...
		bp::extract<mitsuba::Transform> extractTransform(value);
...
static Vector4 Matrix4x4_cholSolve(Matrix4x4 *matrix, Vector B) {
	typedef mitsuba::Matrix<4, 1, Float> Matrix4x1;
	Vector4 X;
...
static Class *object_getClass(mitsuba::Object *object) {
...
static mitsuba::ref<SerializableObject> instance_manager_getinstance(InstanceManager *manager, Stream *stream) {
...
static Vector transform_mul_vector(mitsuba::Transform *transform, const Vector &vector) { return transform->operator()(vector); }
static Vector4 transform_mul_vector4(mitsuba::Transform *transform, const Vector4 &vector) { return transform->operator()(vector); }
static Normal transform_mul_normal(mitsuba::Transform *transform, const Normal &normal) { return transform->operator()(normal); }
static Point transform_mul_point(mitsuba::Transform *transform, const Point &point) { return transform->operator()(point); }
static Ray transform_mul_ray(mitsuba::Transform *transform, const Ray &ray) { return transform->operator()(ray); }
static mitsuba::Transform transform_mul_transform(mitsuba::Transform *transform, const mitsuba::Transform &other) { return *transform * other; }
...
bp::object cast(ConfigurableObject *obj) {
	...
	#define TryCast(ClassName) if (cls->derivesFrom(MTS_CLASS(ClassName))) \
		return bp::object(mitsuba::ref<ClassName>(static_cast<ClassName *>(obj)))
	TryCast(BSDF);
	TryCast(TriMesh);
	TryCast(Shape);
...
static mitsuba::ref<ConfigurableObject> pluginmgr_create_helper(PluginManager *manager, bp::dict dict, std::map<std::string, ConfigurableObject *> &objs) {
	...
	for (bp::stl_input_iterator<bp::tuple> it(items), end; it!=end; ++it) {
		...
		if (name == "type") {
			...
			mitsuba::ref<ConfigurableObject> obj = pluginmgr_create_helper(manager, extractDict(), objs);
			children[name] = obj;
			obj->incRef();
		} else if (extractConfigurableObject.check()) {
			mitsuba::ref<ConfigurableObject> obj = extractConfigurableObject();
			children[name] = obj;
			obj->incRef();
		} else {
	...
	mitsuba::ref<ConfigurableObject> object;
...
static bp::object pluginmgr_create(PluginManager *manager, bp::dict dict) {
	...
	mitsuba::ref<ConfigurableObject> result = pluginmgr_create_helper(manager, dict, objs);
...
static mitsuba::ref<Bitmap> bitmap_convert_3(Bitmap *bitmap, Bitmap::EPixelFormat pixelFormat, Bitmap::EComponentFormat componentFormat,
...
static mitsuba::ref<Bitmap> bitmap_convert_4(Bitmap *bitmap, Bitmap::EPixelFormat pixelFormat, Bitmap::EComponentFormat componentFormat,
		Float gamma, Float multiplier) {
	return bitmap->convert(pixelFormat, componentFormat, gamma, multiplier);
}

static mitsuba::ref<Bitmap> bitmap_convert_5(Bitmap *bitmap, Bitmap::EPixelFormat pixelFormat, Bitmap::EComponentFormat componentFormat,
		Float gamma) {
	return bitmap->convert(pixelFormat, componentFormat, gamma);
}

static mitsuba::ref<Bitmap> bitmap_convert_6(Bitmap *bitmap, Bitmap::EPixelFormat pixelFormat, Bitmap::EComponentFormat componentFormat) {
...
static bp::dict bitmap_split(Bitmap *bitmap) {
	...
	for (std::map<std::string, Bitmap *>::iterator it = map.begin(); it != map.end(); ++it)
		result[it->first] = bp::object(mitsuba::ref<Bitmap>(it->second));
	return result;
}

static bp::object bitmap_plot(Bitmap *_bitmap) {
	bp::dict locals;
	locals["bitmap"] = mitsuba::ref<Bitmap>(_bitmap);
	bp::exec("import matplotlib.pyplot as plt\n"
			 "handle = plt.imshow(bitmap.buffer())\n", bp::object(), locals);
	return locals["handle"];
}

static mitsuba::ref<Bitmap> bitmap_extractChannels(Bitmap::EPixelFormat fmt, Bitmap *bitmap, bp::list list) {
...
static mitsuba::Transform transform_glOrthographic1(Float clipNear, Float clipFar) {
	return mitsuba::Transform::glOrthographic(clipNear, clipFar);
}

static mitsuba::Transform transform_glOrthographic2(Float clipLeft, Float clipRight,
		Float clipBottom, Float clipTop, Float clipNear, Float clipFar) {
	return mitsuba::Transform::glOrthographic(clipLeft, clipRight,
...
struct NativeBuffer {
	mitsuba::ref<mitsuba::Object> owner;
	void *ptr;
	...
	NativeBuffer(mitsuba::Object *owner, void *ptr, Bitmap::EComponentFormat format, int ndim,
...
static mitsuba::ref<Bitmap> bitmap_array_constructor(bp::object _obj) {
	...
	mitsuba::ref<Bitmap> result = new Bitmap(pixelFormat, componentFormat, size, nChannels);
...
void export_core() {
	...
	bp::class_<mitsuba::Object, mitsuba::ref<mitsuba::Object>, boost::noncopyable>("Object", bp::no_init)
		.def("getRefCount", &mitsuba::Object::getRefCount)
		.def("getClass", &object_getClass, BP_RETURN_INTREF)
		.def("__repr__", &mitsuba::Object::toString);
	bp::register_ptr_to_python<mitsuba::Object*>();

	BP_CLASS(Stream, mitsuba::Object, bp::no_init)
	...
	BP_CLASS(SerializableObject, mitsuba::Object, bp::no_init)
	...
	BP_CLASS(Thread, mitsuba::Object, bp::no_init)
	...
	BP_WRAPPED_CLASS(Appender, AppenderWrapper, mitsuba::Object, bp::init<>())
	...
	BP_WRAPPED_CLASS(Formatter, FormatterWrapper, mitsuba::Object, bp::init<>())
	...
	BP_CLASS(Logger, mitsuba::Object, bp::init<ELogLevel>())
	...
	BP_CLASS(InstanceManager, mitsuba::Object, bp::init<>())
	...
	void (Bitmap::*accumulate_1)(const Bitmap *bitmap, Point2i sourceOffset, Point2i targetOffset, mitsuba::Vector2i size) = &Bitmap::accumulate;
	...
	void (Bitmap::*copyFrom_1)(const Bitmap *bitmap, Point2i sourceOffset, Point2i targetOffset, mitsuba::Vector2i size) = &Bitmap::copyFrom;
	...
	mitsuba::ref<Bitmap> (Bitmap::*resample_2)(const ReconstructionFilter *,
		ReconstructionFilter::EBoundaryCondition, ReconstructionFilter::EBoundaryCondition,
		const mitsuba::Vector2i &, Float, Float) const  = &Bitmap::resample;

	void (Bitmap::*filter_1)(const ReconstructionFilter *,
		ReconstructionFilter::EBoundaryCondition, ReconstructionFilter::EBoundaryCondition,
		Bitmap *, Bitmap *, Float, Float) const  = &Bitmap::filter;

	mitsuba::ref<Bitmap> (Bitmap::*filter_2)(const ReconstructionFilter *,
		ReconstructionFilter::EBoundaryCondition, ReconstructionFilter::EBoundaryCondition,
		Float, Float) const  = &Bitmap::filter;

	const std::vector<std::string> & (Bitmap::*getChannelNames_1)() const = &Bitmap::getChannelNames;

	BP_CLASS(Bitmap, mitsuba::Object, (bp::init<Bitmap::EPixelFormat, Bitmap::EComponentFormat, const mitsuba::Vector2i &>()))
		.def(bp::init<Bitmap::EPixelFormat, Bitmap::EComponentFormat, const mitsuba::Vector2i &, int>())
	...
	BP_CLASS(FileResolver, mitsuba::Object, bp::init<>())
	...
	BP_CLASS(Random, mitsuba::Object, bp::init<>())
	...
	BP_CLASS(PluginManager, mitsuba::Object, bp::no_init)
	...
	BP_CLASS(Statistics, mitsuba::Object, bp::no_init)
	...
	BP_CLASS(WorkUnit, mitsuba::Object, bp::no_init)
		.def("set", &WorkUnit::set)
		.def("load", &WorkUnit::load)
		.def("save", &WorkUnit::save);

	BP_CLASS(WorkResult, mitsuba::Object, bp::no_init)
	...
	BP_CLASS(ParallelProcess, mitsuba::Object, bp::no_init)
	...
	BP_CLASS(Scheduler, mitsuba::Object, bp::no_init)
	...
	BP_CLASS(AbstractAnimationTrack, mitsuba::Object, bp::no_init)
	...
	BP_CLASS(AnimatedTransform, mitsuba::Object, (bp::init<mitsuba::Transform>()))
		.def(bp::init<>())
		.def(bp::init<Stream *>())
	...
	BP_NAMESPACE_STRUCT(Color3, bp::init<>())
		.def(bp::init<Float>())
	...
	BP_NAMESPACE_STRUCT(Spectrum, bp::init<>())
	...
	BP_NAMESPACE_STRUCT(Vector1, bp::init<>())
		.def(bp::init< Float>())
		.def(bp::init<Point1>())
		.def_readwrite("x", &Vector1::x);

	BP_NAMESPACE_STRUCT(Vector2, bp::init<>())
		.def(bp::init<Float>())
		.def(bp::init<Float, Float>())
		.def(bp::init<Point2>())
		.def_readwrite("x", &mitsuba::Vector2::x)
		.def_readwrite("y", &mitsuba::Vector2::y);

	BP_NAMESPACE_STRUCT(Vector2i, bp::init<>())
		.def(bp::init<int>())
		.def(bp::init<int, int>())
		.def(bp::init<Point2i>())
		.def_readwrite("x", &mitsuba::Vector2i::x)
		.def_readwrite("y", &mitsuba::Vector2i::y);

	BP_NAMESPACE_STRUCT(Vector3, bp::init<>())
		.def(bp::init<Float>())
		.def(bp::init<Float, Float, Float>())
		.def(bp::init<Point3>())
		.def(bp::init<Normal>())
	...
	BP_SUBSTRUCT(Normal, Vector, bp::init<>())
		.def(bp::init<Float, Float, Float>())
		.def(bp::init<Vector>())
	...
	BP_NAMESPACE_STRUCT(Vector3i, bp::init<>())
		.def(bp::init<int>())
		.def(bp::init<int, int, int>())
		.def(bp::init<Point3i>())
		.def_readwrite("x", &mitsuba::Vector3i::x)
		.def_readwrite("y", &mitsuba::Vector3i::y)
		.def_readwrite("z", &mitsuba::Vector3i::z);

	BP_NAMESPACE_STRUCT(Vector4, bp::init<>())
		.def(bp::init<Float>())
	...
	BP_NAMESPACE_STRUCT(Vector4i, bp::init<>())
		.def(bp::init<int>())
		.def(bp::init<int, int, int, int>())
		.def(bp::init<Point4i>())
		.def_readwrite("x", &mitsuba::Vector4i::x)
		.def_readwrite("y", &mitsuba::Vector4i::y)
		.def_readwrite("z", &mitsuba::Vector4i::z)
		.def_readwrite("w", &mitsuba::Vector4i::w);

	BP_NAMESPACE_STRUCT(Point1, bp::init<>())
		.def(bp::init<Float>())
		.def(bp::init<Vector1>())
		.def_readwrite("x", &Point1::x);

	BP_NAMESPACE_STRUCT(Point2, bp::init<>())
		.def(bp::init<Float>())
		.def(bp::init<Float, Float>())
		.def(bp::init<mitsuba::Vector2>())
		.def_readwrite("x", &Point2::x)
		.def_readwrite("y", &Point2::y);

	BP_NAMESPACE_STRUCT(Point2i, bp::init<>())
		.def(bp::init<int>())
		.def(bp::init<int, int>())
		.def(bp::init<mitsuba::Vector2i>())
		.def_readwrite("x", &Point2i::x)
		.def_readwrite("y", &Point2i::y);

	BP_NAMESPACE_STRUCT(Point3, bp::init<>())
		.def(bp::init<Float>())
		.def(bp::init<Float, Float, Float>())
		.def(bp::init<Vector>())
		.def(bp::init<Normal>())
	...
	BP_NAMESPACE_STRUCT(Point3i, bp::init<>())
		.def(bp::init<int>())
		.def(bp::init<int, int, int>())
		.def(bp::init<mitsuba::Vector3i>())
		.def_readwrite("x", &Point3i::x)
		.def_readwrite("y", &Point3i::y)
		.def_readwrite("z", &Point3i::z);

	BP_NAMESPACE_STRUCT(Point4, bp::init<>())
		.def(bp::init<Float>())
		.def(bp::init<Float, Float, Float, Float>())
		.def(bp::init<mitsuba::Vector4>())
		.def_readwrite("x", &Point4::x)
		.def_readwrite("y", &Point4::y)
		.def_readwrite("z", &Point4::z)
		.def_readwrite("w", &Point4::w);

	BP_NAMESPACE_STRUCT(Point4i, bp::init<>())
		.def(bp::init<int>())
		.def(bp::init<int, int, int, int>())
		.def(bp::init<mitsuba::Vector4i>())
	...
	Float (*dot2)(const mitsuba::Vector2 &, const mitsuba::Vector2 &) = &dot;
	Float (*dot3)(const mitsuba::Vector3 &, const mitsuba::Vector3 &) = &dot;
	Float (*dot4)(const mitsuba::Vector4 &, const mitsuba::Vector4 &) = &dot;
	Float (*absDot2)(const mitsuba::Vector2 &, const mitsuba::Vector2 &) = &absDot;
	Float (*absDot3)(const mitsuba::Vector3 &, const mitsuba::Vector3 &) = &absDot;
	Float (*absDot4)(const mitsuba::Vector4 &, const mitsuba::Vector4 &) = &absDot;
	mitsuba::Vector2 (*normalize2)(const mitsuba::Vector2 &) = &normalize;
	mitsuba::Vector3 (*normalize3)(const mitsuba::Vector3 &) = &normalize;
	mitsuba::Vector4 (*normalize4)(const mitsuba::Vector4 &) = &normalize;
	mitsuba::Vector3 (*cross3)(const mitsuba::Vector3 &, const mitsuba::Vector3 &) = &cross;
	bp::def("dot", dot2);
	bp::def("dot", dot3);
	bp::def("dot", dot4);
	...
	BP_NAMESPACE_STRUCT_DECL(AABB1, bp::init<>());
	BP_IMPLEMENT_AABB_OPS(AABB1, Point1);

	BP_NAMESPACE_STRUCT_DECL(AABB2, bp::init<>());
	BP_IMPLEMENT_AABB_OPS(AABB2, Point2);

	BP_NAMESPACE_STRUCT_DECL(AABB3, bp::init<>());
	...
	BP_NAMESPACE_STRUCT_DECL(AABB4, bp::init<>());
	...
	bp::class_<mitsuba::Transform>("Transform", bp::init<>())
		.def(bp::init<Stream *>())
		.def(bp::init<const Matrix4x4 &>())
		.def(bp::init<const Matrix4x4 &, const Matrix4x4 &>())
		.def("inverse", &mitsuba::Transform::inverse, BP_RETURN_VALUE)
		.def("getMatrix", &mitsuba::Transform::getMatrix, BP_RETURN_VALUE)
		.def("getInverseMatrix", &mitsuba::Transform::getInverseMatrix, BP_RETURN_VALUE)
		.def("det3x3", &mitsuba::Transform::det3x3)
		.def("hasScale", &mitsuba::Transform::hasScale)
		.def("isIdentity", &mitsuba::Transform::isIdentity)
		.def("serialize", &mitsuba::Transform::serialize)
		...
		.def("__repr__", &mitsuba::Transform::toString)
		.def("translate", &mitsuba::Transform::translate, BP_RETURN_VALUE)
		.def("rotate", &mitsuba::Transform::rotate, BP_RETURN_VALUE)
		.def("scale", &mitsuba::Transform::scale, BP_RETURN_VALUE)
		.def("lookAt", &mitsuba::Transform::lookAt, BP_RETURN_VALUE)
		.def("perspective", &mitsuba::Transform::perspective, BP_RETURN_VALUE)
		.def("orthographic", &mitsuba::Transform::orthographic, BP_RETURN_VALUE)
		.def("glPerspective", &mitsuba::Transform::glPerspective, BP_RETURN_VALUE)
		.def("glFrustum", &mitsuba::Transform::glFrustum, BP_RETURN_VALUE)
		.def("glOrthographic", &transform_glOrthographic1, BP_RETURN_VALUE)
		.def("glOrthographic", &transform_glOrthographic2, BP_RETURN_VALUE)
		.def("fromFrame", &mitsuba::Transform::fromFrame, BP_RETURN_VALUE)
	...
	BP_NAMESPACE_STRUCT(Quaternion, bp::init<>())
		.def(bp::init<Vector, Float>())
		.def(bp::init<Stream *>())
		.def_readwrite("v", &mitsuba::Quaternion::v)
		.def_readwrite("w", &mitsuba::Quaternion::w)
		.def(bp::self != bp::self)
		.def(bp::self == bp::self)
		.def(-bp::self)
		.def(bp::self + bp::self)
		...
		.def(bp::self / Float())
		.def(bp::self /= Float())
		.def("isIdentity", &mitsuba::Quaternion::isIdentity)
		.def("axis", &mitsuba::Quaternion::axis)
		.def("angle", &mitsuba::Quaternion::angle)
		.def("exp", &mitsuba::Quaternion::exp)
		.def("log", &mitsuba::Quaternion::log)
		.def("toTransform", &mitsuba::Quaternion::toTransform)
		.def("serialize", &mitsuba::Quaternion::serialize)
		.def("fromAxisAngle", &mitsuba::Quaternion::fromAxisAngle)
		.def("fromTransform", &mitsuba::Quaternion::fromTransform)
		.def("fromDirectionPair", &mitsuba::Quaternion::fromDirectionPair)
		.def("fromMatrix", &mitsuba::Quaternion::fromMatrix)
		.def("fromEulerAngles", &mitsuba::Quaternion::fromEulerAngles)
		.def("__repr__", &mitsuba::Quaternion::toString)
	...
	bp::enum_<mitsuba::Quaternion::EEulerAngleConvention>("EEulerAngleConvention")
		.value("EEulerXYZ", mitsuba::Quaternion::EEulerXYZ)
		.value("EEulerXZY", mitsuba::Quaternion::EEulerXZY)
		.value("EEulerYXZ", mitsuba::Quaternion::EEulerYXZ)
		.value("EEulerYZX", mitsuba::Quaternion::EEulerYZX)
		.value("EEulerZXY", mitsuba::Quaternion::EEulerZXY)
		.value("EEulerZYX", mitsuba::Quaternion::EEulerZYX)
		.export_values();
	BP_SETSCOPE(coreModule);

	Float (*dotQ)(const mitsuba::Quaternion &, const mitsuba::Quaternion &) = &dot;
	mitsuba::Quaternion (*normalizeQ)(const mitsuba::Quaternion &) = &normalize;
	mitsuba::Quaternion (*slerpQ)(const mitsuba::Quaternion &, const mitsuba::Quaternion &, Float) = &slerp;
	...
	BP_NAMESPACE_STRUCT(SHVector, bp::init<>())
		.def(bp::init<int>())
	...
	BP_CLASS(SHSampler, mitsuba::Object, (bp::init<int, int>()))
		.def("warp", &SHSampler::warp);

	BP_NAMESPACE_STRUCT(SHRotation, bp::init<int>())
		.def("__call__", &SHRotation::operator());

	BP_CLASS(Timer, mitsuba::Object, bp::init<bp::optional<bool> >())
	...
	BP_NAMESPACE_STRUCT(VonMisesFisherDistr, bp::init<Float>())
