...
- (void) handleEvent: (NSEvent *) event {
	...
	switch (type) {
		...
		case NSOtherMouseDown: {
				...
				deviceEvent.setMouseRelative(mitsuba::Vector2i(0, 0));
		...
		case NSMouseMoved: {
				...
				if (m_firstMouseMotion) {
					deviceEvent.setMouseRelative(mitsuba::Vector2i(absolute.x, absolute.y));
					m_firstMouseMotion = false;
				} else {
					deviceEvent.setMouseRelative(mitsuba::Vector2i((int) [event deltaX], (int) [event deltaY]));
				}
