...
public:
	...
	void test02_nD_01() {
		...
		assertTrue(quad.integrate(std::bind(
			&TestQuadrature::testF2, this, std::placeholders::_1, std::placeholders::_2),
		&min, &max, &result, &err, &evals) == NDIntegrator::ESuccess);
	...
	void test03_nD_02() {
		...
		assertTrue(quad.integrateVectorized(std::bind(
			&TestQuadrature::testF3, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
			min, max, result, err, &evals) == NDIntegrator::ESuccess);
