...
static void workerThreadFunc(const int tIndex, Thread *parentThread) {
    SAssert(parentThread != nullptr);
    threadIndex = tIndex;
    // Setup Mitsuba's thread data structure
    detail::initializeLocalTLS();
    Thread *thread = Thread::registerUnmanagedThread(formatString("myworker%i", threadIndex));
    SAssert(thread != nullptr);
    thread->setLogger(parentThread->getLogger());
    thread->setFileResolver(parentThread->getFileResolver());
    ...
    // Deinitialize the thread local structure we set up
...
void ParallelFor(const std::function<void(int64_t)> &func, int64_t count, int chunkSize) {
    ...
    if (threads.size() == 0) {
        ...
        for (int i = 0; i < NumSystemCores() - 1; ++i) {
            threads.push_back(std::thread(workerThreadFunc, i + 1, Thread::getThread()));
        }
    }
...
int NumSystemCores() {
    return Scheduler::getInstance()->getCoreCount();
}
