...
static StatsCounter statsSourceFirstDegenerate("Bidirectional mutation",
		"Source first hit degenerate", EPercentage);
static StatsCounter statsProposalFirstDegenerate("Bidirectional mutation",
		"Proposal first hit degenerate", EPercentage);
static StatsCounter statsBothFirstDegenerate("Bidirectional mutation",
		"Both first hit degenerate", EPercentage);
...
bool BidirectionalMutator::sampleMutation(
	...
	int k = source.length();
	if (source.vertex(2)->isDegenerate()) {
		statsSourceFirstDegenerate.incrementBase();
	}
	...
	if (proposal.vertex(2)->isDegenerate()) {
		statsProposalFirstDegenerate.incrementBase();
	}

	if (source.vertex(2)->isDegenerate() &&
			proposal.vertex(2)->isDegenerate()) {
		statsBothFirstDegenerate.incrementBase(1);
	}
	...
	if (source.vertex(2)->isDegenerate()) {
		++statsSourceFirstDegenerate;
	}

	if (proposal.vertex(2)->isDegenerate()) {
		++statsProposalFirstDegenerate;
	}

	if (source.vertex(2)->isDegenerate() &&
		proposal.vertex(2)->isDegenerate()) {
		++statsBothFirstDegenerate;
	}
