...
// Extra namespace to avoid naming conflict
...
struct vertex_key_order : public
	std::binary_function<trimesh::Vertex, trimesh::Vertex, bool> {
	static int compare(const trimesh::Vertex &v1, const trimesh::Vertex &v2) {
	...
	bool operator()(const trimesh::Vertex &v1, const trimesh::Vertex &v2) const {
...
void TriMesh::rebuildTopology(Float maxAngle) {
	typedef std::multimap<trimesh::Vertex, TopoData, vertex_key_order> MMap;
	typedef std::pair<trimesh::Vertex, TopoData> MPair;
	...
	for (MMap::iterator it = vertexToFace.begin(); it != vertexToFace.end();) {
		...
		for (MMap::iterator it2 = start; it2 != end; it2++) {
			const trimesh::Vertex &v = it2->first;
