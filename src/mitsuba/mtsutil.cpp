...
void help() {
	...
	cout <<  "   -m          Execute aether testcases" << endl << endl;
...
int mtsutil(int argc, char **argv) {
	...
	try {
		...
		while ((optchar = getopt(argc, argv, "+a:c:s:n:p:qhwvtm")) != -1) {
			switch (optchar) {
				case 'a': {
		...
		if (testCaseMode || aetherTestMode) {
			std::vector<fs::path> dirPaths = fileResolver->resolveAll("plugins");
			std::set<std::string> seen;
...
#error Unknown operating system!
					...
					std::string shortName = it->path().stem().string();
					if (seen.find(shortName) != seen.end() || 
						(testCaseMode && !boost::starts_with(shortName, "test_")) ||
						(aetherTestMode && !boost::starts_with(shortName, "aether_test_")))
						continue;
					seen.insert(shortName);
					...
					mitsuba::ref<Utility> utility = plugin.createUtility();
