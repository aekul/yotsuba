...
mitsuba::ref<RenderQueue> renderQueue = NULL;
...
int mitsuba_app(int argc, char **argv) {
	...
	try {
		...
		mitsuba::ref<FileResolver> fileResolver = Thread::getThread()->getFileResolver();
		...
		mitsuba::ref<FlushThread> flushThread;
		if (flushTimer > 0) {
		...
		for (int i=optind; i<argc; ++i) {
			...
			mitsuba::ref<FileResolver> frClone = fileResolver->clone();
			...
			mitsuba::ref<Scene> scene = handler->getScene();
			...
			mitsuba::ref<RenderJob> thr = new RenderJob(formatString("ren%i", jobIdx++),
