...
void SceneLoader::run() {
	...
	try {
		...
		if (suffix == "exr" || suffix == "png"  || suffix == "jpg" || suffix == "jpeg" ||
			...
			mitsuba::ref<FileStream> fs = new FileStream(toFsPath(m_filename), FileStream::EReadOnly);
			mitsuba::ref<Bitmap> bitmap = new Bitmap(Bitmap::EAuto, fs, (suffix == "exr") ? "*" : "");
			...
			m_result->scrollOffset = mitsuba::Vector2i(0, 0);
			...
			mitsuba::Vector2i size = scene->getFilm()->getCropSize();
