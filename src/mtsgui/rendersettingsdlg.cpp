...
void RenderSettingsDialog::apply(SceneContext *ctx) {
	...
	mitsuba::ref<mitsuba::Sampler> sampler = static_cast<mitsuba::Sampler *>
		(pluginMgr->createObject(MTS_CLASS(mitsuba::Sampler), samplerProps));
	sampler->configure();
	...
	if (m_integratorNode != NULL)
		m_integratorNode->putProperties(integratorProps);
	mitsuba::ref<Integrator> integrator = static_cast<Integrator *>
	...
	if (ui->icBox->isChecked()) {
		...
		mitsuba::ref<Integrator> ic = static_cast<Integrator *>
	...
	if (ui->aiBox->isChecked()) {
		...
		mitsuba::ref<Integrator> ai = static_cast<Integrator *>
	...
	mitsuba::Vector2i size(mitsuba::math::roundToInt((oldSize.x * cropSize.x / (Float) oldCropSize.x)),
			               mitsuba::math::roundToInt((oldSize.y * cropSize.y / (Float) oldCropSize.y)));

	Point2i cropOffset(mitsuba::math::roundToInt((oldCropOffset.x * cropSize.x / (Float) oldCropSize.x)),
			           mitsuba::math::roundToInt((oldCropOffset.y * cropSize.y / (Float) oldCropSize.y)));
	...
	mitsuba::ref<Film> film = static_cast<Film *> (pluginMgr->createObject(
