...
protected:
	...
	inline mitsuba::Transform getWorldTransform() const {
	...
	inline void setWorldTransform(const mitsuba::Transform &trafo) {
		...
		if (getWorldTransform().det3x3() * trafo.det3x3() > 0)
			...
			getProjectiveCamera()->setWorldTransform(trafo *
				mitsuba::Transform::scale(Vector(-1,1,1)));
	}
...
private:
	...
	mitsuba::ref<Renderer> m_renderer;
	mitsuba::ref<PreviewThread> m_preview;
	mitsuba::ref<GPUTexture> m_logoTexture, m_framebuffer, m_luminanceBuffer[2];
	mitsuba::ref<GPUProgram> m_gammaTonemap, m_reinhardTonemap;
	mitsuba::ref<GPUProgram> m_downsamplingProgram, m_luminanceProgram;
#if MTS_SSE
	mitsuba::ref<TonemapCPU> m_cpuTonemap;
#endif
	mitsuba::ref<QtDevice> m_device;
	mitsuba::ref<Font> m_font;
	mitsuba::ref<Bitmap> m_fallbackBitmap;
	SceneContext *m_context;
	int m_mouseSensitivity;
	mitsuba::Vector2 m_logoSize;
	...
	mitsuba::ref<Timer> m_clock, m_wheelTimer, m_animationTimer;
	mitsuba::ref<Timer> m_statusTimer;
