...
void GLWidget::initializeGL() {
	...
	SAssert(res.isValid());
	mitsuba::ref<Stream> mStream = new MemoryStream(res.size());
	mStream->write(res.data(), res.size());
	mStream->seek(0);
	mitsuba::ref<Bitmap> bitmap = new Bitmap(Bitmap::EPNG, mStream);
	m_logoSize = mitsuba::Vector2(bitmap->getWidth(), bitmap->getHeight());
	m_device->init();
...
void GLWidget::timerImpulse() {
	...
	if (m_animation) {
		...
		setWorldTransform(
			mitsuba::Transform::lookAt(origin, target, m_context->up));
	...
	if (!(m_mouseDrag && m_navigationMode == EStandard) && !m_animation) {
		...
		if (m_leftKeyDown)
			setWorldTransform(getWorldTransform() *
				mitsuba::Transform::translate(Vector(delta,0,0)));
		if (m_rightKeyDown)
			setWorldTransform(getWorldTransform() *
				mitsuba::Transform::translate(Vector(-delta,0,0)));
		if (m_downKeyDown) {
			setWorldTransform(getWorldTransform() *
				mitsuba::Transform::translate(Vector(0,0,-delta)));
...
void GLWidget::mouseMoveEvent(QMouseEvent *event) {
	...
	if (m_cropping) {
		...
		mitsuba::Vector2i maxCrop = m_context->scene->getFilm()->getCropSize();
	...
	mitsuba::Transform invView = getWorldTransform();
	...
	if (event->buttons() & Qt::LeftButton) {
		if (m_navigationMode == EStandard) {
			...
			setWorldTransform(mitsuba::Transform::lookAt(p, target, m_context->up));
		} else {
			...
			mitsuba::Transform trafo = invView
					* mitsuba::Transform::rotate(Vector(1,0,0), pitch)
					* mitsuba::Transform::rotate(Vector(0,1,0), yaw);
			d = trafo(Vector(0,0,1));

			setWorldTransform(mitsuba::Transform::lookAt(p, p+d, up));
		}
		didMove = true;
	} else if (event->buttons() & Qt::MidButton) {
		setWorldTransform(invView *
			mitsuba::Transform::translate(Vector((Float) rel.x(), (Float) rel.y(), 0)
		...
		if (event->modifiers() & Qt::ShiftModifier) {
			...
			m_context->up = mitsuba::Transform::rotate(d, isRightHanded() ? -roll : roll)(up);
			setWorldTransform(mitsuba::Transform::lookAt(p, p+d, m_context->up));
...
Float GLWidget::autoFocus() const {
	...
	mitsuba::Vector2i size = camera->getFilm()->getCropSize();
	ConstShapePtr ptr;
	Normal n;
	Ray ray;
	Point2 uv;
	...
	for (size_t sampleIndex=0; sampleIndex<200; ++sampleIndex) {
		...
		if (scene->rayIntersect(ray, t, ptr, n, uv)) {
			Float weight = mitsuba::math::fastexp(-0.5 / variance * (
...
void GLWidget::paintGL() {
	...
	if (m_context == NULL) {
		...
		mitsuba::Vector2i size = m_context->framebuffer->getSize();
		...
		if (m_context->mode == EPreview) {
			...
			size = mitsuba::Vector2i(entry.buffer->getSize().x, entry.buffer->getSize().y);
			buffer = entry.buffer;
			...
			size = mitsuba::Vector2i(m_framebuffer->getSize().x, m_framebuffer->getSize().y);
		...
		if (m_softwareFallback) {
			...
			mitsuba::Vector2i sourceSize(size.x, size.y);
			m_downsamplingProgram->bind();
			m_downsamplingProgram->setParameter("invSourceSize", mitsuba::Vector2(1.0f/size.x, 1.0f/size.y));
			while (sourceSize != mitsuba::Vector2i(1,1)) {
				target = 1-source;
				mitsuba::Vector2i targetSize((int) std::ceil(sourceSize.x/2.0f), (int) std::ceil(sourceSize.y/2.0f));
				...
				m_downsamplingProgram->setParameter("sourceSize", mitsuba::Vector2(sourceSize.x, sourceSize.y));
				m_downsamplingProgram->setParameter("targetSize", mitsuba::Vector2(targetSize.x, targetSize.y));
			...
			logAvgLuminance = mitsuba::math::fastexp(logAvgLuminance / (size.x*size.y));
		...
		if (m_context->mode == EPreview) {
			...
			if (m_context->showKDTree) {
				...
				mitsuba::Vector2i size = m_font->getSize(message);
		...
		if (!m_statusMessage.empty() && (m_context->mode == EPreview
			...
			m_renderer->setDepthTest(false);
			mitsuba::Vector2i size = m_font->getSize(m_statusMessage);
			m_renderer->setBlendMode(Renderer::EBlendAlpha);
			m_renderer->setColor(Spectrum(0.0f), 0.5f);
...
Point2i GLWidget::upperLeft(bool flipY) const {
	if (!m_context)
		return Point2i(0, 0);
	mitsuba::Vector2i outputSize(m_context->framebuffer->getWidth(),
			m_context->framebuffer->getHeight());
	mitsuba::Vector2i deviceSize = m_device->getSize();
...
void GLWidget::resizeGL(int width, int height) {
	...
	m_device->setSize(mitsuba::Vector2i(width, height));
}
