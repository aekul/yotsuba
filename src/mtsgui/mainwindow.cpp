...
MainWindow::MainWindow(QWidget *parent) :
	...
	connect(m_renderListener, SIGNAL(workCanceled(const RenderJob *, const Point2i &, const mitsuba::Vector2i &)),
		this, SLOT(onWorkCanceled(const RenderJob *, const Point2i &, const mitsuba::Vector2i &)), Qt::DirectConnection);
...
MainWindow::~MainWindow() {
	...
	mitsuba::ref<Scheduler> scheduler = Scheduler::getInstance();
...
bool MainWindow::initWorkersProcessArgv() {
	...
	for (int i=1; i<args.count(); ++i) {
		if (args[i].startsWith("-D")) {
			...
			mitsuba::ref<FileResolver> resolver = Thread::getThread()->getFileResolver();
			QString value = args[i].mid(2);
			if (value.length() == 0 && i+1<args.count())
				value = args[++i];
...
void MainWindow::on_actionImport_triggered() {
	...
	mitsuba::ref<FileResolver> resolver = Thread::getThread()->getFileResolver();
	mitsuba::ref<FileResolver> newResolver = resolver->clone();
...
SceneContext *MainWindow::loadScene(const QString &qFileName, const QString &destFile) {
	mitsuba::ref<FileResolver> resolver = Thread::getThread()->getFileResolver();
	mitsuba::ref<Logger> logger = Thread::getThread()->getLogger();
	...
	mitsuba::ref<SceneLoader> loadingThread;
...
void MainWindow::on_glView_crop(int type, int x, int y, int width, int height) {
	...
	mitsuba::ref<Sensor> oldSensor = scene->getSensor();
	mitsuba::ref<Film> oldFilm = scene->getFilm();

	Properties sensorProps = oldSensor->getProperties();
	Properties filmProps = oldSensor->getFilm()->getProperties();
	mitsuba::Vector2i oldSize = oldFilm->getSize();
	Point2i oldCropOffset = oldFilm->getCropOffset();
	mitsuba::Vector2i oldCropSize = oldFilm->getCropSize();
	...
	if (type == GLWidget::ECrop) {
		...
		width = mitsuba::math::floorToInt(width*magnification);
		height = mitsuba::math::floorToInt(height*magnification);

		filmProps.setInteger("cropOffsetX", mitsuba::math::floorToInt(magnification
				* (x + oldCropOffset.x)), false);
		filmProps.setInteger("cropOffsetY", mitsuba::math::floorToInt(magnification
				* (y + oldCropOffset.y)), false);
		filmProps.setInteger("cropWidth", width);
		filmProps.setInteger("cropHeight", height);
		filmProps.setInteger("width", mitsuba::math::ceilToInt(oldSize.x*magnification), false);
		filmProps.setInteger("height", mitsuba::math::ceilToInt(oldSize.y*magnification), false);
	} else {
	...
	if (oldCropOffset == Point2i(0) &&
		mitsuba::Vector2i(width, height) == oldCropSize)
		return;

	on_tabBar_currentChanged(-1);
	mitsuba::ref<PluginManager> pluginMgr = PluginManager::getInstance();
	mitsuba::ref<Sensor> newSensor = static_cast<Sensor *>
		(pluginMgr->createObject(MTS_CLASS(Sensor), sensorProps));
	mitsuba::ref<Film> newFilm = static_cast<Film *> (
	...
	context->framebuffer = new Bitmap(Bitmap::ERGBA,
		Bitmap::EFloat32, mitsuba::Vector2i(width, height));
	context->framebuffer->clear();
	context->scrollOffset = mitsuba::Vector2i(0, 0);
...
void MainWindow::on_actionSettings_triggered() {
	Logger *logger = Thread::getThread()->getLogger();
	mitsuba::ref<Scheduler> sched = Scheduler::getInstance();
...
void MainWindow::on_actionStop_triggered() {
	...
	if (context->renderJob) {
		...
		mitsuba::ref<RenderJob> renderJob = context->renderJob;
		m_contextMutex.unlock();
		renderJob->cancel();
...
void MainWindow::exportImage(const QString &fileName) {
	if (!fileName.isEmpty()) {
		...
		mitsuba::ref<Bitmap> bitmap = ctx->framebuffer;
		...
		if (fileName == "__clipboard__") {
			...
			mitsuba::ref<FileStream> fs = new FileStream(toFsPath(fileName),
...
void MainWindow::onWorkCanceled(const RenderJob *job, const Point2i &offset, const mitsuba::Vector2i &size) {
	SceneContext *context = getContext(job, false);
	if (context == NULL)
		return;
...
void MainWindow::onActivateCamera() {
	...
	SceneContext *context = m_context[index];
	on_tabBar_currentChanged(-1);
	mitsuba::Vector2i size = sensor->getFilm()->getSize();
...
static mitsuba::ref<Integrator> recursivelyCloneIntegrator(const Integrator *oldIntegrator) {
	mitsuba::ref<Integrator> integrator = static_cast<Integrator *> (PluginManager::getInstance()->createObject(
		MTS_CLASS(Integrator), oldIntegrator->getProperties()));
	int idx = 0;
	while (oldIntegrator->getSubIntegrator(idx) != 0) {
		mitsuba::ref<Integrator> child = recursivelyCloneIntegrator(oldIntegrator->getSubIntegrator(idx));
...
SceneContext::SceneContext(SceneContext *ctx) {
	if (ctx->scene) {
		...
		mitsuba::ref<Thread> thread = Thread::getThread();
		mitsuba::ref<FileResolver> oldResolver = thread->getFileResolver();
		mitsuba::ref<FileResolver> newResolver = oldResolver->clone();
		...
		mitsuba::ref<PluginManager> pluginMgr = PluginManager::getInstance();
		mitsuba::ref<Sensor> oldSensor = static_cast<Sensor *>(ctx->scene->getSensor());
		mitsuba::ref<Sensor> newSensor = static_cast<Sensor *>
			(pluginMgr->createObject(MTS_CLASS(Sensor), oldSensor->getProperties()));
		mitsuba::ref<mitsuba::Sampler> sampler = static_cast<mitsuba::Sampler *>
			(pluginMgr->createObject(MTS_CLASS(mitsuba::Sampler), ctx->scene->getSampler()->getProperties()));
		mitsuba::ref<Film> film = static_cast<Film *>
		...
		mitsuba::ref<ReconstructionFilter> rfilter = static_cast<ReconstructionFilter *>
