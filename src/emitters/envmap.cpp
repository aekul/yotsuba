...
public:
	...
	void configure() {
		...
		const MIPMap::Array2DType &array = m_mipmap->getArray();
		const Vector2i size = array.getSize();
		std::vector<Real> luminanceTable(size.x * size.y);
		int i = 0;
		for (int y = 0; y < size.y; y++) {
			for (int x = 0; x < size.x; x++) {
				const Real weight = std::sin((y + 0.5f) * M_PI / size.y);
				luminanceTable[i++] = (weight * Spectrum(array(x, y)).getLuminance());
			}
		}
		m_emitterDistribution = std::make_shared<Distribution2D>(luminanceTable.data(), size.x, size.y);
		m_sampler = envmap_emitter{this,
								   m_emitterDistribution.get(), 
		 						   m_sceneBSphere.center,
					   			   aether::Real(m_sceneBSphere.radius)};
	}
	...
	Spectrum evalEnvironment(const RayDifferential &ray) const {
		...
		if (!ray.hasDifferentials) {
			...
			// XXX: mitsuba used a hack to smooth out the pdf 
			value = m_mipmap->evalBox(0, uv);
		} else {
...
private:
	...
	std::shared_ptr<Distribution2D> m_emitterDistribution;
