...
public:
	...
	void setParent(ConfigurableObject *parent) {
		...
		if (parent->getClass()->derivesFrom(MTS_CLASS(Shape))) {
			...
			if (!m_shape->isSphere()) {
				m_triMesh = m_shape->createTriMesh();
				SAssert(m_triMesh.get() != nullptr);

				m_trianglePtrs.clear();
				size_t triangleCount = m_triMesh->getTriangleCount();
				const Triangle* trianglePtr = m_triMesh->getTriangles();
				SAssert(trianglePtr != nullptr);
				const Point* vertexPositions = m_triMesh->getVertexPositions();
				const Normal* vertexNormals = m_triMesh->getVertexNormals();
				SAssert(vertexPositions != nullptr);
				m_trianglePtrs.reserve(triangleCount);
				for (size_t i = 0; i < triangleCount; i++) {
					m_trianglePtrs.push_back(trianglePtr++);
				}
				m_triangleWeights.reserve(triangleCount);
				for (size_t i = 0; i < triangleCount; i++) {
					// Sample according to area
					m_triangleWeights.push_back(Real(m_trianglePtrs[i]->surfaceArea(vertexPositions)));
				}
				m_sampler = trimesh_emitter{&m_trianglePtrs, &m_triangleWeights, vertexPositions, vertexNormals};
			} else {
				auto sphere = m_shape->getSphere();
				m_sampler = sphere_emitter{sphere.center, sphere.radius};
			}
		} else {
...
protected:
	...
	ref<TriMesh> m_triMesh;
	std::vector<const Triangle*> m_trianglePtrs;
	std::vector<Real> m_triangleWeights;
	emitter_composite_t m_sampler;
};
